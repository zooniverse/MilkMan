...
class Subject
  ...
  def annotations
    ...
    task_keys = ["illustration", "parts", "details"]
  ...
  def image_size
    begin
      return FastImage.size(self.image[0], :raise_on_failure=>true)
    rescue
      return [self.metadata["original_size"]["width"], self.metadata["original_size"]["height"]]
    end
  end
